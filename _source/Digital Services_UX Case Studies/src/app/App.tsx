...
import { StarbucksFlowShowcaseSlide } from '@/app/components/slides/StarbucksFlowShowcaseSlide';
...
import ExperisUXcs from '@/app/Experis_UX_cs';
...
function App() {
  ...
  // Route to Experis UX CS if hash is #experis
  if (currentRoute === '#experis') {
    return <ExperisUXcs />;
  }

  // Default route - main presentation with deck switcher
...
function MainPresentation() {
  ...
  const showStarbucks = true; // All decks now show Starbucks (redacted in deck2 and liteRedacted)
  const chapters: SlideChapter[] = [
    ...
    { index: 24, label: deckConfig[activeDeck].isRedacted ? 'Coffee Loyalty' : 'Starbucks Loyalty', color: 'bg-rose-500' },
    { index: 30, label: deckConfig[activeDeck].isRedacted ? 'Disaster Tool' : 'Walmart DRT', color: 'bg-blue-600' },
  ];
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Press 'ArrowUp' to cycle through modes (Dark-R -> Dark -> Lite-R -> Lite -> Dark-R)
      if (e.key === 'ArrowUp') {
        const modes = ['deck2', 'deck1', 'liteRedacted', 'lite'] as const;
        const currentIndex = modes.indexOf(activeDeck);
        const nextIndex = (currentIndex + 1) % modes.length;
        handleDeckSwitch(modes[nextIndex]);
        return;
      }
      
      // Press 'ArrowDown' to cycle through modes in reverse
      if (e.key === 'ArrowDown') {
        const modes = ['deck2', 'deck1', 'liteRedacted', 'lite'] as const;
        const currentIndex = modes.indexOf(activeDeck);
        const prevIndex = (currentIndex - 1 + modes.length) % modes.length;
        handleDeckSwitch(modes[prevIndex]);
        return;
      }
  ...
  }, [currentSlide, showBrandAssets, activeDeck]);
  ...
  const slides = [
    ...
    <CaseStudyIndexSlide
      ...
      caseStudies={[
        ...
        {
          title: `${content.companies.starbucks} Loyalty App UX Research`,
          categories: ['UX Research', 'Strategic Advisory', 'Product Strategy'],
          slideNumber: 25,
    ...
    // =====================================
    // CASE STUDY 4: Starbucks (6 slides)
    // Shown in all decks (redacted in deck2 and liteRedacted)
    // =====================================
    <TitleSlide
      key="starbucks-title"
      category={['UX Research', 'Strategic Advisory', 'Product Strategy']}
      title={`${content.companies.starbucks} Loyalty App UX Research`}
      subtitle="Comprehensive research and strategic recommendations for improving retention"
      backgroundImage={
        deckConfig[activeDeck].isRedacted 
          ? "https://images.unsplash.com/photo-1758840734307-aed01ccec284?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtb2Rlcm4lMjBjb2ZmZWUlMjBzaG9wJTIwcGVvcGxlJTIwbGFwdG9wcyUyMHdvcmtpbmclMjBjYWZlJTIwaW50ZXJpb3J8ZW58MXx8fHwxNzcxMjc2NTQ0fDA&ixlib=rb-4.1.0&q=80&w=1080"
          : "https://images.unsplash.com/photo-1545231027-637d2f6210f8?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzdGFyYnVja3MlMjBjb2ZmZWUlMjBhcHB8ZW58MXx8fHwxNzY4ODUwNjI5fDA&ixlib=rb-4.1.0&q=80&w=1080"
      }
    />,
    ...
    <ScopeSlide
      key="starbucks-scope"
      title="Project Scope"
      client={content.companies.starbucks}
      industry="Food & Beverage Retail"
      duration="11 months"
      teamSize="8 UX researchers, 6 designers"
      overview={`As the world's largest coffeehouse chain with 35,000+ locations globally, ${content.companies.starbucks} needed to understand why their loyalty app—despite millions of downloads—had declining retention rates. Our mission was to conduct comprehensive UX research to identify barriers and provide strategic recommendations to improve customer engagement and long-term retention.`}
      deliverables={[
        'Multi-phase UX research study with 120+ participants',
        'User journey maps and behavioral analysis',
        'Strategic design recommendations and advisory',
        'UX improvement roadmap with prioritized initiatives',
      ]}
    />,

    <TimelineSlide
      key="starbucks-timeline"
      title="Project Timeline & Methodology"
      subtitle="11-month UX research initiative"
      challengePoints={[
        'App had 25M downloads but 60-day retention was only 38%',
        'Rewards program perceived as confusing and not valuable enough',
        'Mobile ordering experience causing friction and abandoned orders',
        'Lack of personalization made app feel generic despite customer data',
      ]}
      approachPoints={[
        'Conducted mixed-methods research: surveys, interviews, usability tests, analytics',
        'Built interactive data visualizations to communicate behavioral patterns',
        'Developed evidence-based strategic recommendations backed by user data',
        'Delivered comprehensive research report with prioritized UX improvement roadmap',
      ]}
      phases={[
        {
          phase: 'UX Research',
          duration: '4 months',
          activities: [
            'User interviews with 120+ participants',
            'Survey deployment to 5,000+ active users',
            'In-app behavioral analytics deep dive',
            'Contextual inquiry & competitor benchmarking',
          ],
        },
        {
          ...
          duration: '3 months',
          activities: [
            'Journey mapping across user segments',
            'Pain point identification and prioritization',
            'Conceptual wireframes for recommended solutions',
        ...
        },
        {
          ...
          duration: '3 months',
          activities: [
            'UX improvement roadmap development',
            'Design recommendations with rationale',
            'Prioritization framework for initiatives',
            'Implementation feasibility assessment',
          ],
        },
        {
          phase: 'Delivery & Advisory',
          duration: '1 month',
          activities: [
            'Comprehensive research report delivery',
            'Strategic recommendations presentation',
            'Executive stakeholder workshops',
            'Ongoing advisory support and guidance',
          ],
        },
      ]}
    />,
    ...
    <UserFlowChartSlide
      key="starbucks-journey"
      title="User Flow & Drop-off Analysis"
      subtitle="Identifying friction points and optimization opportunities in the loyalty experience"
      accentColor="emerald"
      flowData={{
        id: 'start',
        label: 'App Open',
        ...
        children: [
            ...
            children: [
              {
                ...
                children: [
                  {
                    id: 'rewards-confusion',
                    label: 'Rewards Unclear',
                    percentage: 22,
                    type: 'dropoff',
                    dropoffRate: 62,
                    children: [
                      ...
                      {
                        id: 'rewards-redesign',
                        label: 'Recommended: Visual Progress',
                        percentage: 54,
                        type: 'sticky',
                        stickiness: 87,
                        showcaseComponent: (
                          <div className="w-full h-full flex items-center justify-center">
                            <div className="relative">
                              {/* Phone mockup frame */}
                              <div className="w-[340px] h-[680px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden relative">
                                {/* Status bar */}
                                <div className="absolute top-0 left-0 right-0 h-10 bg-gradient-to-b from-black/40 to-transparent z-10 flex items-center justify-between px-8 pt-2">
                                ...
                                {/* App content */}
                                <div className="h-full bg-gradient-to-br from-emerald-900 via-emerald-800 to-slate-900 p-6 pt-16">
                                  <div className="mb-6">
                                    <h2 className="text-3xl font-bold text-white mb-1">Rewards</h2>
                                    <p className="text-emerald-200 text-sm">You're almost there!</p>
                                  </div>

                                  {/* Main progress card */}
                                  <div className="bg-gradient-to-br from-amber-400 via-amber-500 to-orange-500 rounded-3xl p-6 shadow-xl mb-6">
                                    <div className="flex items-center justify-between mb-4">
                                      <div>
                                        <p className="text-amber-900 text-sm font-semibold mb-1">PROGRESS TO REWARD</p>
                                        <p className="text-white text-3xl font-bold">127 ★</p>
                                      </div>
                                      <div className="text-right">
                                        <p className="text-amber-900 text-sm font-semibold mb-1">GOAL</p>
                                        <p className="text-white text-3xl font-bold">150 ★</p>
                                      </div>
                                    </div>
                                    <div className="relative">
                                      <div className="w-full bg-amber-600/40 rounded-full h-4 overflow-hidden">
                                        <div className="bg-white h-4 rounded-full shadow-lg" style={{ width: '85%' }}></div>
                                      </div>
                                      <div className="absolute -top-1 left-[85%] transform -translate-x-1/2">
                                        <div className="w-6 h-6 bg-white rounded-full shadow-lg flex items-center justify-center">
                                          <Star className="w-3 h-3 text-amber-500 fill-amber-500" />
                                        </div>
                                      </div>
                                    </div>
                                    <p className="text-amber-900 text-sm font-semibold mt-4 text-center">23 stars until free drink! ☕</p>
                                  </div>

                                  {/* Achievement badges */}
                                  <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/20">
                                    <p className="text-white text-sm font-semibold mb-4">Recent Achievements</p>
                                    <div className="grid grid-cols-5 gap-3">
                                      {[1, 2, 3, 4, 5].map((i) => (
                                        <div key={i} className="relative">
                                          <div className={`aspect-square rounded-xl flex items-center justify-center ${i <= 3 ? 'bg-gradient-to-br from-emerald-400 to-emerald-600 shadow-lg shadow-emerald-500/50' : 'bg-slate-700/50 border-2 border-dashed border-slate-600'}`}>
                                            <Star className={`w-6 h-6 ${i <= 3 ? 'text-white fill-white' : 'text-slate-500'}`} />
                                          </div>
                                          {i <= 3 && (
                                  ...
                                  <div className="absolute bottom-6 left-6 right-6">
                                    <div className="bg-emerald-500/20 backdrop-blur-lg border border-emerald-400/30 rounded-2xl p-4">
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                          <TrendingUp className="w-5 h-5 text-emerald-400" />
                                          <span className="text-white text-sm font-semibold">Engagement Impact</span>
                                        </div>
                                        <span className="text-emerald-400 text-xl font-bold">+87%</span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              ...
                              {/* Feature label */}
                              <div className="absolute -right-20 top-8 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-4 py-2 rounded-lg shadow-lg">
                                <div className="flex items-center gap-2">
                ...
                children: [
                  {
                    ...
                    children: [
                      ...
                      {
                        id: 'reorder-old',
                        label: 'Try to Reorder Usual',
                        percentage: 16,
                        type: 'action',
                        children: [
                          {
                            id: 'reorder-friction',
                            label: 'Too Many Steps (8 taps)',
                            percentage: 7,
                            type: 'dropoff',
                            dropoffRate: 56,
                          },
                              ...
                              <div className="w-full h-full flex items-center justify-center">
                                <div className="relative">
                                  ...
                                  <div className="w-[340px] h-[680px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden relative">
                                    ...
                                    {/* App content */}
                                    <div className="h-full bg-gradient-to-br from-slate-900 via-slate-800 to-purple-900/50 p-6 pt-16">
                                      <div className="mb-6">
                                        <h2 className="text-3xl font-bold text-white mb-1">Order</h2>
                                        <p className="text-slate-300 text-sm">Your usual is ready</p>
                                      </div>

                                      {/* Quick Reorder Card - HERO */}
                                      <div className="bg-gradient-to-br from-purple-600/90 via-purple-700/90 to-purple-800/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl mb-6 border-2 border-purple-400/50 relative overflow-hidden">
                                        ...
                                        <div className="relative">
                                          <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <span className="text-purple-100 text-xs font-bold uppercase tracking-wide">Quick Reorder</span>
                                          </div>
                                          
                                          <div className="flex items-center gap-4 mb-5">
                                            <div className="w-20 h-20 bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl flex items-center justify-center shadow-xl flex-shrink-0">
                                              <span className="text-4xl">☕</span>
                                            </div>
                                            <div className="flex-1">
                                              <h3 className="text-white text-xl font-bold mb-1">{content.products.starbucksExample.charAt(0).toUpperCase() + content.products.starbucksExample.slice(1)}</h3>
                                              <p className="text-purple-200 text-sm">Extra shot • Oat milk • Light ice</p>
                                            </div>
                                          </div>

                                          <div className="flex items-center gap-3 mb-4">
                                            <div className="flex-1 bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                                              <p className="text-purple-200 text-xs mb-1">Pick up at</p>
                                              <p className="text-white text-sm font-semibold">Downtown Seattle</p>
                                            </div>
                                            <div className="flex-1 bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                                              <p className="text-purple-200 text-xs mb-1">Ready in</p>
                                              <p className="text-white text-sm font-semibold">~5 mins</p>
                                            </div>
                                          </div>

                                          <button className="w-full bg-white text-purple-700 rounded-2xl py-4 font-bold text-lg shadow-2xl hover:bg-purple-50 transition-all flex items-center justify-center gap-2">
                                      ...
                                      <div className="space-y-3 opacity-50">
                                        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10 flex items-center gap-3">
                                          ...
                                          <div className="flex-1">
                                            <p className="text-white text-sm font-semibold">Butter Croissant</p>
                                      ...
                                      {/* Impact metric */}
                                      <div className="absolute bottom-6 left-6 right-6">
                                        <div className="bg-purple-500/20 backdrop-blur-lg border border-purple-400/30 rounded-2xl p-4">
                                          <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                              <Clock className="w-5 h-5 text-purple-300" />
                                              <span className="text-white text-sm font-semibold">Time Saved</span>
                                            </div>
                                            <span className="text-purple-300 text-xl font-bold">8 → 1 tap</span>
                                          </div>
                                          <p className="text-purple-200 text-xs mt-2">68% faster checkout</p>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  {/* Feature label */}
                                  <div className="absolute -right-20 top-8 bg-gradient-to-r from-purple-600 to-purple-700 text-white px-4 py-2 rounded-lg shadow-lg">
                                    <div className="flex items-center gap-2">
                                      <Sparkles className="w-4 h-4" />
                                      <span className="text-sm font-semibold">NEW</span>
                                    </div>
          ...
          {
            id: 'notifications',
            label: 'From Push Notification',
            percentage: 4,
            type: 'action',
            children: [
              {
                id: 'notification-offer',
                label: 'View Personalized Offer',
                percentage: 3.6,
                type: 'sticky',
                stickiness: 240,
                showcaseComponent: (
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="relative">
                      ...
                      <div className="w-[340px] h-[680px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden relative">
                        ...
                        {/* App content */}
                        <div className="h-full bg-gradient-to-br from-slate-900 via-orange-950/20 to-slate-900 p-6 pt-16">
                          <div className="flex items-center justify-between mb-4">
                            <div>
                              <h2 className="text-2xl font-bold text-white mb-1">For You</h2>
                              <p className="text-slate-300 text-xs">Special offers today</p>
                            </div>
                          ...
                          <div className="relative mb-4">
                            {/* Animated glow */}
                            <div className="absolute inset-0 bg-gradient-to-r from-orange-400 to-amber-400 rounded-3xl blur-xl opacity-40 animate-pulse"></div>
                            
                            <div className="relative bg-gradient-to-br from-orange-500/90 via-orange-600/90 to-amber-600/90 rounded-2xl p-4 shadow-2xl overflow-hidden">
                              {/* Background pattern */}
                              ...
                              <div className="relative">
                                <div className="flex items-center gap-2 mb-2">
                                  <Gift className="w-4 h-4 text-orange-50" />
                                  <span className="text-orange-50 text-xs font-bold uppercase tracking-wide">Personalized for You</span>
                                </div>

                                <h3 className="text-white text-2xl font-bold mb-1">50% Off</h3>
                                <p className="text-orange-50 text-base mb-1">Your Favorite Drink!</p>
                                <p className="text-orange-100 text-xs mb-3">{content.products.starbucksExample.charAt(0).toUpperCase() + content.products.starbucksExample.slice(1)}</p>

                                {/* Drink preview */}
                                <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 mb-3 flex items-center gap-3">
                                  <div className="w-12 h-12 bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-lg flex items-center justify-center shadow-lg flex-shrink-0">
                                    <span className="text-2xl">☕</span>
                                  </div>
                                  <div className="flex-1">
                                    <p className="text-white text-xs font-bold">Was $5.45</p>
                                    <p className="text-white text-xl font-bold">Now $2.73</p>
                                  </div>
                                </div>

                                {/* Timer */}
                                <div className="bg-white/20 backdrop-blur-sm rounded-lg p-2 flex items-center justify-between">
                                  <span className="text-orange-50 text-xs">Expires in</span>
                                  <div className="flex items-center gap-1">
                                    <Clock className="w-3 h-3 text-white" />
                                    <span className="text-white text-xs font-bold">3h 24m</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          {/* CTA Button - Outside orange card */}
                          <div className="mb-4 px-1">
                            <button className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl py-3 font-bold text-base shadow-2xl hover:from-orange-600 hover:to-amber-600 transition-all">
                              Redeem Offer
                            </button>
                          </div>
                          ...
                          {/* Impact metric */}
                          <div className="absolute bottom-6 left-6 right-6">
                            <div className="bg-orange-500/20 backdrop-blur-lg border border-orange-400/30 rounded-2xl p-4">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <Target className="w-5 h-5 text-orange-300" />
                                  <span className="text-white text-sm font-semibold">ML-Driven Targeting</span>
                                </div>
                                <span className="text-orange-300 text-xl font-bold">+240%</span>
                              </div>
                              <p className="text-orange-200 text-xs mt-2">Conversion rate increase</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Feature label */}
                      <div className="absolute -right-20 top-8 bg-gradient-to-r from-orange-600 to-amber-600 text-white px-4 py-2 rounded-lg shadow-lg">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4" />
                          <span className="text-sm font-semibold">NEW</span>
                        </div>
    ...
    <StarbucksFlowShowcaseSlide
      key="starbucks-design-showcase"
      title="Strategic Design Recommendations"
      subtitle="Research-backed UX improvements with measurable impact projections"
      accentColor="emerald"
      popoutComponents={[
        {
          title: 'Recommended: Visual Rewards Progress',
          description: 'Research showed users need clear progress indicators. Recommended star-based gamification system with visual feedback showing proximity to rewards. Testing suggests 87% improvement in engagement.',
          color: 'emerald',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-br from-blue-50/90 via-sky-50/85 to-blue-100/80 rounded-2xl p-3.5 shadow-2xl border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)]">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-lg">
                    <Star className="w-3.5 h-3.5 text-white fill-white" />
                  </div>
                  <span className="text-sm font-bold text-slate-900">Rewards</span>
                </div>
                <div className="text-[11px] font-semibold text-emerald-600 bg-emerald-100/80 px-2.5 py-1 rounded-full animate-pulse">3 of 5 ★</div>
              </div>
                  ...
                  <div 
                    ...
                    className={`w-9 h-9 rounded-full ${
                      i < 3 
                        ...
                        : 'bg-gradient-to-br from-slate-50/80 to-slate-100/60 shadow-sm'
                    } flex items-center justify-center`}
                  >
                    <Star className={`w-4 h-4 ${i < 3 ? 'text-white fill-white' : 'text-slate-400 fill-slate-400'}`} />
                  </div>
                ))}
              </div>
              <div className="relative w-full h-2.5 bg-gradient-to-r from-slate-50/80 to-slate-100/60 rounded-full overflow-hidden shadow-inner mb-1.5">
                <div className="absolute inset-0 w-[60%] h-full bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-full shadow-lg"></div>
                <div className="absolute right-[40%] top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-lg border-2 border-emerald-500 animate-pulse"></div>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-slate-800 font-medium">2 more for free drink!</span>
        ...
        {
          title: 'Recommended: Personalized Offers',
          description: 'Users want relevant offers. Recommend AI-driven personalization using purchase history and behavioral data. Research suggests potential 240% increase in redemption rates.',
          color: 'blue',
          component: (
            <div className="flex flex-col gap-2">
              <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="flex-1 bg-gradient-to-br from-blue-50/85 via-sky-50/75 to-blue-100/70 border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)] rounded-2xl p-3 shadow-xl hover:shadow-2xl transition-shadow flex items-center">
                <div className="flex items-center gap-3 w-full">
                  <div className="relative w-11 h-11 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/40 flex-shrink-0">
                    <Gift className="w-5 h-5 text-white" />
                    <div className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-gradient-to-br from-amber-400 to-amber-500 rounded-full flex items-center justify-center shadow-md">
                      <Sparkles className="w-2.5 h-2.5 text-white animate-pulse" />
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="text-sm font-bold text-slate-900">Free Pastry</div>
                    <div className="text-[10px] text-slate-800 font-medium">With any drink</div>
                  </div>
                  <div className="text-[9px] text-blue-600 font-semibold bg-blue-100/80 px-2 py-1 rounded-md">3 days</div>
                </div>
              </div>
              <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="flex-1 bg-gradient-to-br from-blue-50/85 via-sky-50/75 to-blue-100/70 border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)] rounded-2xl p-3 shadow-xl hover:shadow-2xl transition-shadow flex items-center">
                <div className="flex items-center gap-3 w-full">
                  <div className="relative w-11 h-11 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg shadow-purple-500/40 flex-shrink-0">
                    <Star className="w-5 h-5 text-white fill-white" />
                    <div className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-gradient-to-br from-rose-400 to-rose-500 rounded-full flex items-center justify-center shadow-md text-white text-[9px] font-bold">50</div>
                  </div>
                  <div className="flex-1">
                    <div className="text-sm font-bold text-slate-900">Bonus Stars</div>
                    <div className="text-[10px] text-slate-800 font-medium">This week only</div>
                  </div>
                  <div className="text-[9px] text-purple-600 font-semibold bg-purple-100/80 px-2 py-1 rounded-md">You</div>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'Recommended: Quick Reorder',
          description: 'Research identified friction in ordering flow. Recommend quick-action tile for repeat orders to eliminate 4+ taps. Estimated 68% reduction in order completion time.',
          color: 'emerald',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-br from-blue-50/85 via-sky-50/75 to-blue-100/70 border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)] rounded-2xl p-3 shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02] cursor-pointer flex items-center">
              <div className="flex items-center gap-2.5 w-full">
                <div className="w-11 h-11 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/40">
                  <Coffee className="w-5 h-5 text-white" />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-bold text-slate-900">Reorder Usual</div>
                  <div className="text-[10px] text-slate-800 font-medium">Grande Latte</div>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'Scan & Pay',
          description: 'In-store mobile payment reduces checkout friction and wait times.',
          color: 'amber',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-br from-blue-50/85 via-sky-50/75 to-blue-100/70 border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)] rounded-2xl p-3 shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02] cursor-pointer flex items-center">
              <div className="flex items-center gap-2.5 w-full">
                <div className="w-11 h-11 bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/40">
                  <Zap className="w-5 h-5 text-white" />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-bold text-slate-900">Scan & Pay</div>
                  <div className="text-[10px] text-slate-800 font-medium">In-store</div>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'Mobile Order Tracking',
          description: 'Real-time order status with estimated preparation time reduces anxiety and wait-time uncertainty. Push notifications alert users when orders are ready for pickup.',
          color: 'amber',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-br from-blue-50/90 via-sky-50/85 to-blue-100/80 rounded-2xl p-3.5 shadow-2xl border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)]">
              <div className="flex items-center gap-2.5 mb-2">
                <div className="w-10 h-10 bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/40">
                  <Clock className="w-5 h-5 text-white" />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-bold text-slate-900">Order in Progress</div>
                  <div className="text-[11px] text-slate-800 font-medium">Ready in ~4 min</div>
                </div>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full flex items-center justify-center shadow-md">
                    <CheckCircle className="w-4 h-4 text-white" />
                  </div>
                  <div className="flex-1">
                    <div className="text-xs font-semibold text-slate-900">Order Received</div>
                    <div className="text-[9px] text-slate-700 font-medium">9:23 AM</div>
                  </div>
                </div>
                <div className="ml-3.5 w-0.5 h-3 bg-gradient-to-b from-emerald-500 to-amber-500"></div>
                <div className="flex items-center gap-2.5">
                  <div className="relative w-7 h-7 bg-gradient-to-br from-amber-500 to-amber-600 rounded-full flex items-center justify-center shadow-md">
                    <div className="absolute inset-0 rounded-full bg-amber-500 animate-ping opacity-40"></div>
                    <Coffee className="w-3.5 h-3.5 text-white" />
                  </div>
                  <div className="flex-1">
                    <div className="text-xs font-semibold text-slate-900">Preparing</div>
                    <div className="text-[9px] text-amber-600 font-semibold">Barista: Alex</div>
                  </div>
                </div>
                <div className="ml-3.5 w-0.5 h-3 bg-slate-100/60"></div>
                <div className="flex items-center gap-2.5 opacity-50">
                  <div className="w-7 h-7 bg-slate-100/60 rounded-full flex items-center justify-center">
                    <CheckCircle className="w-4 h-4 text-slate-400" />
                  </div>
                  <div className="flex-1">
                    <div className="text-xs font-semibold text-slate-600">Ready for Pickup</div>
                    <div className="text-[9px] text-slate-500 font-medium">Waiting...</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {
          title: 'Recommended: Smart Store Finder',
          description: 'Users struggle finding nearby locations. Recommend location-aware store finder with real-time wait times and amenity filters (drive-thru, mobile order, seating).',
          color: 'rose',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-br from-blue-50/90 via-sky-50/85 to-blue-100/80 rounded-2xl p-3.5 shadow-2xl border border-blue-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.95)]">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 bg-gradient-to-br from-rose-500 to-rose-600 rounded-lg flex items-center justify-center shadow-lg shadow-rose-500/40">
                    <MapPin className="w-3.5 h-3.5 text-white animate-pulse" />
                  </div>
                  <span className="text-sm font-bold text-slate-900">Nearby Stores</span>
                </div>
                <div className="text-[9px] text-rose-600 font-semibold bg-rose-100/80 px-2 py-0.5 rounded-md">3 open</div>
              </div>
              <div className="space-y-1.5">
                <div style={{ backdropFilter: 'blur(96px)', WebkitBackdropFilter: 'blur(96px)' }} className="bg-gradient-to-br from-blue-100/75 via-sky-100/65 to-blue-200/60 border border-blue-300/50 rounded-xl p-2.5 shadow-md">
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex-1">
                      <div className="text-xs font-bold text-slate-900">Main & 5th Ave</div>
                      <div className="text-[9px] text-slate-700 font-medium">0.2 mi • 3 min</div>
                    </div>
                    <div className="flex items-center gap-1 bg-emerald-100 px-1.5 py-0.5 rounded-full">
                      <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse"></div>
                      <span className="text-[8px] text-emerald-700 font-semibold">Low</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="text-[8px] text-slate-600 bg-slate-100/70 px-1.5 py-0.5 rounded">Mobile</div>
                    <div className="text-[8px] text-slate-600 bg-slate-100/70 px-1.5 py-0.5 rounded">Drive-thru</div>
                  </div>
                </div>
                <div style={{ backdropFilter: 'blur(96px)', WebkitBackdropFilter: 'blur(96px)' }} className="bg-gradient-to-br from-blue-100/75 via-sky-100/65 to-blue-200/60 border border-blue-300/50 rounded-xl p-2.5 shadow-sm">
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex-1">
                      <div className="text-xs font-bold text-slate-900">Park Plaza</div>
                      <div className="text-[9px] text-slate-700 font-medium">0.5 mi • 7 min</div>
                    </div>
                    <div className="flex items-center gap-1 bg-amber-100 px-1.5 py-0.5 rounded-full">
                      <div className="w-1 h-1 bg-amber-500 rounded-full"></div>
                      <span className="text-[8px] text-amber-700 font-semibold">Med</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="text-[8px] text-slate-600 bg-slate-100/70 px-1.5 py-0.5 rounded">Mobile</div>
                    <div className="text-[8px] text-slate-600 bg-slate-100/70 px-1.5 py-0.5 rounded">Seating</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        },
        {
          ...
          description: 'Time-based notifications deliver relevant reward expiration alerts without overwhelming users.',
          color: 'purple',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-r from-purple-500/95 to-purple-600/95 rounded-xl px-3 py-2 shadow-lg border border-purple-300/50">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-white/25 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
                  <Bell className="w-3.5 h-3.5 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-white">Reward Expiring</span>
                    <span className="text-[9px] text-white/70 font-medium">2h</span>
                  </div>
                  <p className="text-[10px] text-white/90 font-medium">Free drink expires in 3 days!</p>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'New Seasonal',
          description: 'Personalized seasonal drink suggestions based on user preferences and behavior patterns.',
          color: 'emerald',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-r from-emerald-500/95 to-emerald-600/95 rounded-xl px-3 py-2 shadow-lg border border-emerald-300/50">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-white/25 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-white">New Seasonal</span>
                    <span className="text-[9px] text-white/70 font-medium">8:30</span>
                  </div>
                  <p className="text-[10px] text-white/90 font-medium">Pumpkin Spice Latte for you!</p>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'Order Ready',
          description: 'Real-time order completion alerts with pickup location details.',
          color: 'blue',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-r from-blue-500/95 to-blue-600/95 rounded-xl px-3 py-2 shadow-lg border border-blue-300/50">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-white/25 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-white">Order Ready!</span>
                    <span className="text-[9px] text-white/70 font-medium">now</span>
                  </div>
                  <p className="text-[10px] text-white/90 font-medium">Pick up at counter</p>
                </div>
              </div>
            </div>
          ),
        ...
        {
          title: 'Double Stars Today',
          description: 'Limited-time promotional alerts for bonus star earning opportunities.',
          color: 'amber',
          component: (
            <div style={{ backdropFilter: 'blur(64px)', WebkitBackdropFilter: 'blur(64px)' }} className="bg-gradient-to-r from-amber-500/95 to-amber-600/95 rounded-xl px-3 py-2 shadow-lg border border-amber-300/50">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-white/25 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-white">Double Stars Today</span>
                    <span className="text-[9px] text-white/70 font-medium">12h</span>
                  </div>
                  <p className="text-[10px] text-white/90 font-medium">Earn 2X on all purchases</p>
                </div>
              </div>
            </div>
          ),
    ...
    <PerformanceIntegrationSlide
      key="starbucks-results"
      title="Projected Impact"
      subtitle="Estimated results based on research findings and prototype testing"
      stats={[
        { value: '14.7M', label: 'Monthly Active Users', icon: Users, gradient: 'from-blue-500 to-cyan-500' },
        { value: '71%', label: '60-Day Retention', icon: TrendingUp, gradient: 'from-violet-500 to-purple-500' },
        { value: '4.8★', label: 'App Store Rating', icon: Star, gradient: `${content.brandColors.starbucksGradientFrom} ${content.brandColors.starbucksGradientTo}` },
      ]}
      performanceItems={[
        {
          label: '60-Day Retention Rate',
          before: '38%',
          after: '71%',
          improvement: '+87% projected',
        },
        {
          label: 'App Store Rating',
          before: '3.9 stars',
          after: '4.8 stars',
          improvement: '+0.9 stars projected',
        },
        {
          label: 'Average Order Value',
          before: '$6.20',
          after: '$9.40',
          improvement: '+52% projected',
        },
        {
          label: 'Monthly Active Users',
          before: '8.2M',
          after: '14.7M',
          improvement: '+79% projected',
        },
        {
          label: 'One-Tap Reorder Usage',
          before: 'N/A',
          after: '62%',
          improvement: '68% faster (estimated)',
        },
        {
          label: 'Personalized Offer Conversion',
          before: '12%',
          after: '41%',
          improvement: '+240% projected',
        },
      ]}
    />,
  ...
  const handleDeckSwitch = (deck: 'deck1' | 'deck2' | 'lite' | 'liteRedacted') => {
    setActiveDeck(deck);
    // Keep current slide when switching decks
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden flex items-center justify-center ${deckConfig[activeDeck].isLiteMode ? 'lite-mode' : 'bg-slate-900'}`}>
      ...
      {/* Minimal Deck Switcher - Top Left Corner */}
      <div 
      ...
      >
        ...
        <div 
          className={`absolute top-0 left-0 bg-slate-800/95 backdrop-blur-xl border border-slate-700/50 rounded-lg px-2 py-2 shadow-xl flex flex-col gap-1.5 transition-all duration-300 ${
